...
class PutFilmServiceTest {
    ...
    @Test
    public void deveAlterarADescricaoComSucesso() throws FilmNotFoundException {


        PutFilmeRequest request = mockPutFilmesRequest();
        ...
        filmeSave.setVersion(2);
        filmeSave.setOpeningCrawl( request.getOpeningCrawl() );
        when(filmeRepository.save(filmeSave)).thenReturn(filmeSave);

        PutFilmeResponse response = putFilmService.update(VALID_ID, request);


        assertEquals(request.getOpeningCrawl(), response.getOpeningCrawl());
    }
